...
public class GestioneRistorante {
	...
	public static void showFirstMenu(Utente x, int userID) {
		...
		while (!exit) {
			...
			switch (input) {
				case 1: {
					showOrderMenu(ristorante,x, userID);
					break;
				}
				case 2:
					exit = true;
	...
	public static void showOrderMenu(Ristorante ristorante, Utente x, int userID) {
		...
		while (!exitOrderMenu) {
			...
			switch (inputOrder) {
				...
				case 1:
					...
					ristorante.ordina(input,x,userID);
					break;
				case 2:
					System.out.println(ristorante.toString());
